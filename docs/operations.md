...
smallest bucket/prefix and operations required. Pin SFTP host keys. Verify FTP
certificates and SNI. Error messages must not include passwords, secret keys,
authorization headers, signed query strings, or private keys.
...
Use `fstest.NewFaultReader` and `fstest.NewFaultIterator` for short reads,
disconnects, malformed pages, and cleanup assertions. The memory adapter is
appropriate for domain tests only when the domain does not depend on a weaker
backend guarantee.
