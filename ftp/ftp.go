// Package ftp provides a capability-based FTP and FTPS adapter. The external
...
const (
	// TLSExplicit upgrades a plaintext control connection with AUTH TLS.
	TLSExplicit TLSMode = iota
	// TLSImplicit starts TLS immediately, normally on port 990.
	TLSImplicit
...
type Config struct {
	...
	Root string
	// TLSMode selects explicit, implicit, or explicitly allowed plaintext FTP.
	TLSMode TLSMode
...
func New(ctx context.Context, configuration Config) (*Adapter, error) {
	...
	if configuration.TLSMode != TLSPlaintext {
		tlsConfiguration := configuration.TLSConfig.Clone()
		if tlsConfiguration.MinVersion == 0 {
			tlsConfiguration.MinVersion = tls.VersionTLS12
		}
		if tlsConfiguration.ServerName == "" {
			host, _, splitErr := net.SplitHostPort(configuration.Address)
			if splitErr != nil {
				return nil, errors.New("ftp: address must include a port")
			}
			tlsConfiguration.ServerName = host
		}
		if configuration.TLSMode == TLSExplicit {
			options = append(options, protocol.WithExplicitTLS(tlsConfiguration))
		} else {
			options = append(options, protocol.WithImplicitTLS(tlsConfiguration))
		}
	}
